...
io.on("connect", (socket) =>{
    socket.on("room", (roomData, callback) =>{
        const user = getTools.addUser(roomData.name, roomData.room, socket.id);
        if(user.error){
    ...
    socket.on("location", (location, callback) =>{
        ...
        io.to(user.room).emit("message", getTools.generateMessage(url, user.username));
        callback();
    });
    ...
    socket.on("disconnect", () =>{
        ...
        io.to(user.room).emit("message", getTools.generateMessage(`${user.username} has left!`, "System"));
